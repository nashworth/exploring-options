...
void setup()
{
  ...
  {
    xos = nvmobj.calx;            // update offset values from EEPROM
    yos = nvmobj.caly;
    zos = nvmobj.calz;
  }
...
void loop()
{
  for(int f = 0; f < 8; f++)      // step through 8 frequencies starting with ODR = 25Hz
  {
    ...
    for (int i=1; i<32; i++)    // read all 32 values from arrays to calculate min, max, avg
    {
...
// calculate amplitude by summing the squares, divide vectors by 100 to keep number manageable
      ...
      xavg += xdata[i];     // add samples for average, only useful for calibrating
      yavg += ydata[i];
      zavg += zdata[i];
    }
    Adata[f] = sqrt(Adata[f]);  // take squareroot of sum of the squares
    xavg = xavg>>5;             // divide average sums by 32 to get averages
    yavg = yavg>>5;
    zavg = zavg>>5;
  }
...
int configAcc(String command)     // Particle function for accelerometer calibration
{
    if (command == "calibrate")   // if function call = "calibrate", then offsets = averages
    {
      ...
      nvmobject nvmobj = { 0xaa55, xos, yos, zos};
